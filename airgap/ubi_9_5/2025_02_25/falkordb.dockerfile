...
RUN git clone --recurse-submodules -j8 --depth 1 --branch v4.8.3 https://github.com/FalkorDB/FalkorDB.git;
RUN cd FalkorDB; make
RUN gzip -c /home/maker/FalkorDB/bin/linux-x64-release/src/falkordb.so  > /airgap/archive/falkordb_v4.8.3.so.gz
